 ...
 ** 电平控制简化_宏定义
 ***********************************************/
#define SCL_L      (I2C_MONI_SCL_GPIO6->BSRR = (I2C_MONI_SCL_PIN6 << 16))
#define SCL_H      (I2C_MONI_SCL_GPIO6->BSRR =  I2C_MONI_SCL_PIN6)
#define SDA_L      (I2C_MONI_SDA_GPIO6->BSRR = (I2C_MONI_SDA_PIN6 << 16))
#define SDA_H      (I2C_MONI_SDA_GPIO6->BSRR =  I2C_MONI_SDA_PIN6)
#define SDA_READ   ((I2C_MONI_SDA_GPIO6->IDR & I2C_MONI_SDA_PIN6)?1:0)       // 不用改成输入模式就能读
...
*备  注：
*****************************************************************************/
void IICSoft_Init6(void)
{
    // 使能SCL引脚端口时钟
    if(I2C_MONI_SCL_GPIO6 == GPIOA)   __HAL_RCC_GPIOA_CLK_ENABLE();
    if(I2C_MONI_SCL_GPIO6 == GPIOB)   __HAL_RCC_GPIOB_CLK_ENABLE();
    if(I2C_MONI_SCL_GPIO6 == GPIOC)   __HAL_RCC_GPIOC_CLK_ENABLE();
    if(I2C_MONI_SCL_GPIO6 == GPIOD)   __HAL_RCC_GPIOD_CLK_ENABLE();
    if(I2C_MONI_SCL_GPIO6 == GPIOE)   __HAL_RCC_GPIOF_CLK_ENABLE();
    // 使能SDA引脚端口时钟
    if(I2C_MONI_SDA_GPIO6 == GPIOA)  __HAL_RCC_GPIOA_CLK_ENABLE();
    if(I2C_MONI_SDA_GPIO6 == GPIOB)  __HAL_RCC_GPIOB_CLK_ENABLE();
    if(I2C_MONI_SDA_GPIO6 == GPIOC)  __HAL_RCC_GPIOC_CLK_ENABLE();
    if(I2C_MONI_SDA_GPIO6 == GPIOD)  __HAL_RCC_GPIOD_CLK_ENABLE();
    if(I2C_MONI_SDA_GPIO6 == GPIOE)  __HAL_RCC_GPIOF_CLK_ENABLE();

    // 配置引脚工作模式: 开漏输出
    GPIO_InitTypeDef G;
    G.Pin   = I2C_MONI_SCL_PIN6;
    G.Mode  = GPIO_MODE_OUTPUT_OD;
    G.Speed = 50000000 ;
    HAL_GPIO_Init(I2C_MONI_SCL_GPIO6, &G);
    delayUs(2);

    G.Pin   = I2C_MONI_SDA_PIN6;
    HAL_GPIO_Init(I2C_MONI_SDA_GPIO6, &G);
    delayUs(2);
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadByte6(uint8_t slave, uint8_t addr, uint8_t *buf)
{
    start ();                     // 起始信号
 ...
 * 备  注： 注意，时序有点特别
*****************************************************************************/
uint8_t IICSoft_ReadBueffer6(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())              // 等待ACK
 ...
 * 备  注： 时序与读是不同的
*****************************************************************************/
uint8_t IICSoft_WriteByte6(uint8_t slave, uint8_t addr, uint8_t data)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
// 往从机写多个字节数据
uint8_t IICSoft_WriteBuffer6(uint8_t slave, uint8_t addr, uint8_t *buf, uint8_t len)
{
    start ();                     // 起始信号
    sendByte (slave<<1 | 0);      // 从机地址,　写方向 , 0写1读
    if(waitForAck())
...
uint8_t detectMagnet6(void)
{
    ...
    IICSoft_ReadByte6(_ams5600_Address,_stat,&magStatus);
    detectMag6++;
...
int16_t getRawAngle6(void)
{
    int getRaw6=0;
    uint8_t Raw6[2]={0,0};
    IICSoft_ReadBueffer6(_ams5600_Address, _raw_ang_hi, (uint8_t *) Raw6, 2);
    getRaw6 = (Raw6[0] << 8 | Raw6[1]);
    return getRaw6;
...
******************************************************/
float convertRawAngleToDegrees6(int16_t newAngle)
{
...
float Programe_Run6(void)
{
    float degree6=0;
    float rawdata6=0;

    rawdata6 = getRawAngle6();
    degree6 = convertRawAngleToDegrees6(rawdata6);
